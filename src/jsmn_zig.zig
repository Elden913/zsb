    ...
    return struct {
        ...
        pub fn compressTokens(parsed: []const Token, parsed_count: usize, out: []CompactToken) !usize {
            ...
            for (parsed[0..parsed_count]) |t| {
                ...
                // Для великих JSON використовуємо тільки стандартні токени
                if (start32 >= (1 << 20)) {
                    // Не можемо упакувати - пропускаємо компактні токени для цього випадку
                    continue;
                }
                ...
                if (t.typ == .Object) {
                    ...
                    while (rem > 0) : (rem -= 2) {
                        ...
                        if (child < out.len) {
                            ...
                            const new_key = try packCompact(
                                compactGetType(key_token),
                                true,
                                compactGetStart(key_token),
                                compactGetLen(key_token)
                            );
                            out[child] = new_key;
                        }
        ...
        pub fn parseHybrid(allocator: ?std.mem.Allocator, input: []const u8) !HybridResult {
            ...
            if (allocator == null or est <= 512) {
                ...
                if (USE_COMPACT and input.len < (1 << 20)) {
                    ...
                    const ccount = try compressTokens(&std_tokens, used, &compact_buf);
                ...
                if (USE_COMPACT and input.len < (1 << 20)) {
                    ...
                    const ccount = try compressTokens(tmp_tokens, used, compact_tmp);
